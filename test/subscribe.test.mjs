...
test('rejects an empty email', async () => {
  ...
  const res = await subscribeFlow('   ', CFG, () => { throw new Error('must not fetch'); });
...
test('rejects an email without an @', async () => {
  assert.ok(subscribeFlow, 'subscribeFlow not found in index.html');
  const res = await subscribeFlow('sam.sixtom.com', CFG, () => { throw new Error('must not fetch'); });
  assert.equal(res.state, 'invalid');
});

test('reports early when no listmonk is configured', async () => {
  assert.ok(subscribeFlow, 'subscribeFlow not found in index.html');
  const res = await subscribeFlow('sam@sixtom.com', { url: '', list: '' }, () => { throw new Error('must not fetch'); });
...
test('posts the trimmed email and list uuid to the listmonk public API', async () => {
  assert.ok(subscribeFlow, 'subscribeFlow not found in index.html');
  const calls = [];
  const fetchFn = (url, opts) => { calls.push({ url, opts }); return Promise.resolve({ ok: true }); };
  ...
  assert.deepEqual(JSON.parse(calls[0].opts.body), { email: 'sam@sixtom.com', list_uuids: ['uuid-123'] });
});
...
test('failed on a non-2xx response', async () => {
  assert.ok(subscribeFlow, 'subscribeFlow not found in index.html');
  const res = await subscribeFlow('sam@sixtom.com', CFG, () => Promise.resolve({ ok: false, status: 500 }));
...
test('failed when the network throws', async () => {
  assert.ok(subscribeFlow, 'subscribeFlow not found in index.html');
  const res = await subscribeFlow('sam@sixtom.com', CFG, () => Promise.reject(new Error('offline')));
  assert.equal(res.state, 'failed');
});
